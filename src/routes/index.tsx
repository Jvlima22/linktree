...
import { SignalNews } from "@/components/SignalNews";
...
const SOCIALS = [
  {
    name: "LinkedIn",
    href: "https://www.linkedin.com/in/josu%C3%A9-lima-67b2843b3/",
    color: "#0A66C2",
    icon: (
  ...
  {
    name: "Instagram",
    href: "https://www.instagram.com/jotadev.ai/",
    color: "#E1306C",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-5">
...
function BioPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-4 pb-28 md:max-w-md md:mx-auto selection:bg-accent/30">
        ...
        <div className="flex items-center justify-center gap-3">
          {SOCIALS.map((s) => (
            <a
              ...
              aria-label={s.name}
              style={{ color: s.color, ["--brand" as any]: s.color }}
              className="size-11 rounded-xl bg-card border border-border flex items-center justify-center hover:border-[var(--brand)]/50 hover:bg-[var(--brand)]/10 transition-colors"
            >
              {s.icon}
            </a>
          ))}
      ...
      <section className="mt-12 space-y-4 animate-entrance" style={{ animationDelay: "650ms" }}>
        <SignalNews />
      </section>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-md bg-card/90 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl p-2 flex items-center justify-between gap-2 z-50">
        <button
          data-cal-link="jotadev.ai/30min"
          data-cal-config='{"theme":"dark"}'
